...
for i, tau in enumerate(MATURITIES):
    ax.plot(MONEYNESS, heston_ivs[i] * 100,
            color=colors[i], linewidth=2.2, marker='o', markersize=6,
            label=f'Heston  $\\tau = {tau}$y')

ax.axhline(bs_flat_vol * 100, color='black', linestyle='--', linewidth=1.8,
           label=f'Black-Scholes flat $\\sigma = {bs_flat_vol*100:.0f}\\%$')
ax.axvline(0, color='grey', linestyle=':', linewidth=1.0, alpha=0.7)
ax.annotate('ATM', xy=(0.01, heston_ivs.min() * 100 + 0.2),
            fontsize=9, color='grey')

ax.set_xlabel('Log-Moneyness  $\\ln(K/S_0)$')
ax.set_ylabel('Implied Volatility (\\%)')
ax.set_title('Heston Implied Volatility Smile vs Black-Scholes Flat Volatility')
ax.legend(ncol=2, framealpha=0.92)
ax.grid(True, alpha=0.3)
ax.set_xlim(MONEYNESS[0] - 0.02, MONEYNESS[-1] + 0.02)

plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, 'fig01_iv_smile.pdf'), bbox_inches='tight')
