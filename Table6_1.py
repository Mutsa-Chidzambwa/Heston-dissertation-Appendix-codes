import numpy as np
from scipy.integrate import quad
import warnings
warnings.filterwarnings('ignore')

# True parameters θ*
KAPPA   = 2.0
THETA   = 0.04
SIGMA_V = 0.30
RHO     = -0.70
V0      = 0.04
R       = 0.05
S0      = 100.0

# ATM contract
K_ATM = 100.0
TAU   = 1.0
N     = 200     # time steps per path

# Semi-analytical price (Albrecher et al. branch-cut-safe) 
def heston_cf(u, S0, V0, kappa, theta, sigma_v, rho, r, tau):
    i   = 1j
    d   = np.sqrt((kappa - i*rho*sigma_v*u)**2 + sigma_v**2*(u**2 + i*u))
    b   = kappa - i*rho*sigma_v*u
    g   = (b - d) / (b + d)
    D   = ((b - d) / sigma_v**2
           * (1 - np.exp(-d*tau)) / (1 - g*np.exp(-d*tau)))
    C   = (i*u*r*tau
           + kappa*theta/sigma_v**2
           * ((b-d)*tau - 2*np.log((1 - g*np.exp(-d*tau))/(1 - g))))
    return np.exp(C + D*V0 + i*u*np.log(S0))

def sa_price(S0, K, r, tau, V0, kappa, theta, sigma_v, rho):
    i = 1j
    def i1(u):
        return np.real(
            np.exp(-i*u*np.log(K))
            * heston_cf(u-i, S0, V0, kappa, theta, sigma_v, rho, r, tau)
            / heston_cf(-i,  S0, V0, kappa, theta, sigma_v, rho, r, tau)
            / (i*u))
    def i2(u):
        return np.real(
            np.exp(-i*u*np.log(K))
            * heston_cf(u, S0, V0, kappa, theta, sigma_v, rho, r, tau)
            / (i*u))
    P1 = 0.5 + quad(i1, 1e-8, 200, limit=200, epsabs=1e-8)[0] / np.pi
    P2 = 0.5 + quad(i2, 1e-8, 200, limit=200, epsabs=1e-8)[0] / np.pi
    return S0*P1 - K*np.exp(-r*tau)*P2

# Standard MC (no variance reduction)
def mc_standard(M, seed=42):
    np.random.seed(seed)
    dt  = TAU / N
    Z1  = np.random.randn(M, N)
    Z2  = RHO*Z1 + np.sqrt(1 - RHO**2)*np.random.randn(M, N)
    V   = np.full(M, V0, float)
    S   = np.full(M, S0, float)
    for t in range(N):
        Vp  = np.maximum(V, 0.0)
        V   = V + KAPPA*(THETA - Vp)*dt + SIGMA_V*np.sqrt(Vp*dt)*Z2[:, t]
        S   = S * np.exp((R - 0.5*Vp)*dt + np.sqrt(Vp*dt)*Z1[:, t])
    disc   = np.exp(-R*TAU)
    payoff = disc * np.maximum(S - K_ATM, 0.0)
    price  = payoff.mean()
    se     = payoff.std(ddof=1) / np.sqrt(M)
    return price, se

# Antithetic variates MC
def mc_antithetic(M, seed=42):
    np.random.seed(seed)
    dt  = TAU / N
    Z1  = np.random.randn(M, N)
    Z2  = RHO*Z1 + np.sqrt(1 - RHO**2)*np.random.randn(M, N)

    def simulate(z1, z2):
        V = np.full(M, V0, float)
        S = np.full(M, S0, float)
        for t in range(N):
            Vp  = np.maximum(V, 0.0)
            V   = V + KAPPA*(THETA - Vp)*dt + SIGMA_V*np.sqrt(Vp*dt)*z2[:, t]
            S   = S * np.exp((R - 0.5*Vp)*dt + np.sqrt(Vp*dt)*z1[:, t])
        return S

    ST_orig = simulate( Z1,  Z2)
    ST_anti = simulate(-Z1, -Z2)

    disc   = np.exp(-R*TAU)
    payoff = disc * 0.5*(np.maximum(ST_orig - K_ATM, 0.0)
                       + np.maximum(ST_anti - K_ATM, 0.0))
    price  = payoff.mean()
    se     = payoff.std(ddof=1) / np.sqrt(M)
    return price, se

# Run 
print("=" * 65)
print("TABLE 6.1: Effect of Antithetic Variates on MC Standard Error")
print(f"ATM Call:  K = {K_ATM},  tau = {TAU}y,  N = {N} time steps")
print("=" * 65)

SA = sa_price(S0, K_ATM, R, TAU, V0, KAPPA, THETA, SIGMA_V, RHO)
print(f"\nSemi-analytical benchmark price = {SA:.4f}")
print()

# Header
print(f"{'Method':<25} {'Paths M':>8} {'SA Price':>10} "
      f"{'Std Error':>11} {'SE Reduction':>13}")
print("-" * 70)

for M in [50_000, 25_000]:
    p_std,  se_std  = mc_standard(M)
    p_anti, se_anti = mc_antithetic(M)
    reduction = (se_std - se_anti) / se_std * 100.0

    print(f"{'Without antithetic':<25} {M:>8,} {SA:>10.4f} "
          f"{se_std:>11.4f} {'---':>13}")
    print(f"{'With antithetic':<25} {M:>8,} {SA:>10.4f} "
          f"{se_anti:>11.4f} {f'~{reduction:.0f}%':>13}")
    print()
    
